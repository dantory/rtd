...
const play = (place, useMeta) => [
  ...
  '    let g=0;',
  /* 상한 60 — 자에 걸린 값으로는 벽이 어디인지 못 읽는다. 30 이면 상성이 들어간 뒤 열 판 중
     셋이 걸렸고, 40 은 전리품이 들어간 뒤 뒤 절반이 통째로 걸려 두 봇을 구분하지 못했다. */
  /* **상한이 자를 자르면 안 된다.** 60 으로 두었더니 뒤쪽 판이 55~56R 에 닿아,
     아무거나와 골라서의 차이가 천장에 눌려 안 보였다(+0.3 으로 보였다). */
  '    while(!S.over && S.round<=140 && g++<900){',

...
/* ══ 뽑기의 희귀도 — **대박도 꽝도 없으면 뽑을 맛이 없다** ══
   열두 종을 균등하게 뽑고 있었다. 그러면 무엇이 나와도 감정이 같다 — 나올 것이
   나온 것뿐이라 "떴다"가 없다. 종류마다 무게를 달리 주면 그때부터 **귀한 게 뜬 판과
   안 뜬 판이 갈린다.** 귀한 쪽은 한 자리를 통째로 바꾸는 것들(로켓·지휘관)로 둔다 —
   희귀도가 세기와 안 맞으면 그건 그냥 성가신 확률표일 뿐이다. */
export const RARE = { 0:{ n:"흔함", col:"#8a8f98", w:6 },
                      1:{ n:"드묾", col:"#6ba8d6", w:3 },
                      2:{ n:"귀함", col:"#e0a458", w:1 } };
/* **역할 유닛(위생병·방패병)은 흔한 쪽에 둔다.** 한 번 드묾으로 올려 봤더니
   counteraudit 이 +3.8R 에서 **-2.5R** 로 뒤집혔다 — 예고를 읽어도 갈아 끼울 것이
   손에 없으면 읽을 값이 없다. 살아남는 수단을 운에 걸면 대박이 아니라 짜증이다.
   귀한 쪽은 **없어도 죽지는 않지만 있으면 판이 달라지는 것**으로 둔다. */
/* **테마마다 흔한 답이 하나씩은 있어야 한다.** 「두꺼움」의 답은 "한 방이 큰 공격"인데
   그게 지뢰병·로켓병·저격수뿐이라 셋 다 흔하지 않으면, 예고를 읽어도 손에 답이 없다
   (실제로 counteraudit 이 -2R 로 뒤집혔다). 지뢰병을 흔한 쪽으로 내린다.
   빠름→냉동병 · 떼/분열→폭탄·화염·전격 · 방패→소총·화염 · 치유/포격/자폭→전격병 은
   이미 흔한 쪽에 답이 있다. */
const RARITY = { gun:0, cannon:0, frost:0, bolt:0, flame:0, medic:0, guard:0, mine:0,
                 rail:1, drone:1,
                 rocket:2, officer:2 };
...
/* **첫 판은 빨리 죽어야 한다.**(병수님: "초반 난이도가 너무 쉬운데,, 초반에 어렵게
   만들어서 바로 실패했음 좋겠어, 초반도 성장을 좀 해야 깨지게")
   520 이면 아무것도 안 사고도 12~15 라운드를 간다 — 첫 실패까지 몇 분이 걸리고,
...
/*  **한 라운드가 10초면 짧다**(병수님, 2026-08-07). 첫 판 전체가 72초였다.
 *  이유는 초반 웨이브가 얇아서다 — R1 이 여덟 마리면 4초 만에 다 나오고, 남은 건
 *  걸어오는 시간뿐이라 배치를 볼 짬도 없다. 그래서 **마릿수를 앞으로 당기고 마리당
 *  체력을 낮춰** 총량은 그대로 두었다: R1 이 여덟에서 열셋으로, 한 마리는 110 에서 72 로.
 *  R1 이 12~15초, 첫 판 전체가 93초가 된다(뒤쪽 라운드는 20~40초).
 *
 *  줄어든 뒤쪽 총량은 **체력이 아니라 마릿수로** 메운다(2.4 → 2.6). 체력 증가율을 1.30 으로
...
/** **이미 깬 데를 또 기어오르지 않는다.**
 *
 *  인크리멘털의 판은 벽에 부딪히고 강화하고 다시 오는 반복인데, 그 "다시"가 매번 1라운드
 *  부터면 벽에 닿기까지 몇 분이 앞에 붙는다(병수님: "적군이 많이 나올때까지 테스트하기가
 *  어렵네"). 최고 기록보다 **열 라운드 아래**까지 건너뛸 수 있게 한다 — 열 라운드는 남겨서
 *  손이 풀릴 자리와 배치를 바꿔 볼 자리를 준다.
 *  **눈금은 10 단위**다(병수님). 5 단위로 뒀더니 「25라운드부터」·「35라운드부터」처럼
 *  버튼에 적히는 수가 판마다 들쭉날쭉해, 같은 자리에서 다시 시작한다는 느낌이 안 났다.
 *  건너뛴 라운드의 **판 도중 전리품은 안 들어온다**(그 라운드를 안 돌았으니까). 정산 자원은
 *  도달 라운드로 셈하므로 그대로다 — 그래서 건너뛰기는 이득이 아니라 **시간을 아끼는 것**이다. */
export const skipTo = () => Math.max(0, Math.floor((META.best - 10) / 10) * 10);
